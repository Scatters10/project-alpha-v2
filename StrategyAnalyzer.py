...
def main():
    ...
    # Report name for output files (optional 4th argument)
    ...
    if report_name:
        ...
        chart_file = os.path.join(REPORTS_DIR, f"{report_name}_chart.png")
        report_file = os.path.join(REPORTS_DIR, f"{report_name}_report.txt")
        trades_file = os.path.join(REPORTS_DIR, f"{report_name}_trades.json")
    else:
        chart_file = "chart.png"
        report_file = DEFAULT_REPORT_FILE
        trades_file = DEFAULT_TRADE_FILE

    # Optional override: load from a provided JSON file instead of fetching
    # Check if arg2 is a JSON file (ends with .json)
    json_file = None
    if len(sys.argv) > 2:
        arg2 = sys.argv[2].strip()
        if arg2 and arg2.endswith('.json'):
            json_file = arg2
    ...
    if json_file:
        ...
        # Support command-line args: market_query and user_address
        # arg2 = market_query, arg3 = user_address (if resolved_arg provided)
        # OR arg1 = market_query, arg2 = user_address (if no resolved_arg)
        if resolved_arg:
            market_query = sys.argv[2] if len(sys.argv) > 2 else None
            user_address = sys.argv[3] if len(sys.argv) > 3 else None
        else:
            market_query = sys.argv[1] if len(sys.argv) > 1 else None
        ...
        print(f"Saved {len(raw_data)} trades to {trades_file}")
    ...
    print(f"Stats report saved as {report_file}")
